...
export const TextInputBuilder = ({ componentEdit = {} }) => {
  ...
  const handleSubmit = () => {
    if (questionText) {
      ...
      const newComponent = createComponentObject(questionText, 'textInput');
  ...
  const handleUpdate = () => {
    if (questionText) {
      ...
      if (index !== -1) {
        componentListFinal[index] = createComponentObject(questionText, 'textInput');
      } else {
